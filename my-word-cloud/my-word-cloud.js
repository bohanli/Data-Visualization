...
MyWordCloud.prototype = 
{
	run: function()
	{
		...
		for( var i = 0; i < words.length; i++)
		{
			...
			thisWord.fontSize = Math.floor( thisWord.weight * 6 ) ;
			ctx.font = thisWord.fontSize + "px " + this.font;
			thisWord.textWidth = Math.floor( ctx.measureText(thisWord.word).width ) + 10;
		...
		for( var i = 0; i < words.length; i++)
		{
			...
			thisWord.position = {};
			//thisWord.position.x = Math.floor( canvas.width / 2 - canvas.width / 6 + Math.random() * canvas.width / 6 );
			//thisWord.position.y = Math.floor( canvas.height / 2 - canvas.height / 6 + Math.random() * canvas.height / 6 );
			thisWord.position.x = Math.floor( canvas.width / 2 );
			thisWord.position.y = Math.floor( canvas.height / 2 );

			// random direction
			//thisWord.direction = ( Math.random() > 0.5 ) ? 1 : 0; // 0: horizon 1: vertical
			thisWord.direction = 0;
			//if( i > 2 * words.length / 3 )
			//	thisWord.direction = ( Math.random() > 0.5 ) ? 1 : 0; // 0: horizon 1: vertical
			//thisWord.direction = 1;
			...
			while( ++tryCnt < tryMax )
			{
				...
				// move position spirally
				//t += Math.random() * Math.PI / 360 * 3;
